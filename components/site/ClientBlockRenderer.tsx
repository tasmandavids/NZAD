...
// ============================================================================
//  components/site/ClientBlockRenderer.tsx
//  Client-safe block renderer — shared by the public site wrapper and the editor.
// ============================================================================
...
import { BLOCK_WRAP, BlockShell } from "@/lib/site/block-shell";
import { type Block, type BlockProps, type BlockType } from "@/lib/site/blocks";
...
function HeadingBlock({ p, labels }: { p: BlockProps; labels: PublicLabels }) {
  ...
  return (
    <BlockShell p={p} type="heading">
      <div className={`${BLOCK_WRAP} ${align}`}>
        {level === "h1" ? (
...
function LinkBlock({ p, labels }: { p: BlockProps; labels: PublicLabels }) {
  ...
  return (
    <BlockShell p={p} type="linkBlock">
      <div className={`${BLOCK_WRAP} flex ${align}`}>
        {variant === "text" ? (
...
function RichText({ p }: { p: BlockProps }) {
  ...
  return (
    <BlockShell p={p} type="richText">
      <div className={`${BLOCK_WRAP} ${center ? "text-center" : ""}`}>
        {str(p, "heading") && (
...
function DividerBlock({ p }: { p: BlockProps }) {
  ...
  return (
    <BlockShell p={p} type="divider" className="!py-4">
      <div className={`${BLOCK_WRAP} flex ${justify}`}>
        <hr
